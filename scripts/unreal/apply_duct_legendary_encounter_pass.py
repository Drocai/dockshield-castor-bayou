...
MATERIAL_DIR = "/Game/DockShield/Environment/Duct/Materials"
RESET_PREFIX = "DS_Duct_"
...
MATERIAL_SPECS = {
    "M_DS_DuctRubberYellow_Proto": {
        "base": unreal.LinearColor(1.0, 0.72, 0.04, 1.0),
        "roughness": 0.16,
        "metallic": 0.0,
        "emissive": unreal.LinearColor(0.60, 0.36, 0.02, 1.0),
    },
    "M_DS_DuctTapeSilver_Proto": {
        "base": unreal.LinearColor(0.42, 0.44, 0.40, 1.0),
        "roughness": 0.24,
        "metallic": 0.55,
        "emissive": None,
    },
    "M_DS_DuctWakeGold_Proto": {
        "base": unreal.LinearColor(0.95, 0.60, 0.02, 1.0),
        "roughness": 0.08,
        "metallic": 0.12,
        "emissive": unreal.LinearColor(1.0, 0.56, 0.02, 1.0),
    },
}


def fail(message):
    raise RuntimeError(f"DockShield Duct legendary encounter pass failed: {message}")
...
def spawn_static(label, location, rotation, scale, mesh, material):
    ...
    actor.set_folder_path("DockShield/Duct")
...
def spawn_point_light(label, location, color, intensity, radius):
    ...
    light.set_folder_path("DockShield/Duct/Lighting")
    ...
    if component:
        ...
        set_prop(component, "source_radius", 40.0)
    return light
...
def apply_pass():
    ...
    duct_class = getattr(unreal, "DSDuctLegendaryEncounterActor", None)
    if duct_class is None:
        fail("DSDuctLegendaryEncounterActor class is unavailable. Build DockShieldEditor first.")

    duct = ACTOR_EDITOR.spawn_actor_from_class(
        duct_class,
        unreal.Vector(820.0, -520.0, 96.0),
        unreal.Rotator(0.0, 25.0, 0.0),
    )
    duct.set_actor_label("DS_Duct_Legendary_Sighting_Debug")
    duct.set_folder_path("DockShield/Duct")
    duct.tags = [unreal.Name("DockShieldTarget"), unreal.Name("Duct"), unreal.Name("LegendaryDuct")]
    set_mesh(duct, sphere, unreal.Vector(0.34, 0.24, 0.20))
    set_material(duct, materials["M_DS_DuctRubberYellow_Proto"])

    targetable_cls = getattr(unreal, "DSTargetableComponent", None)
    if targetable_cls:
        targetable = duct.get_component_by_class(targetable_cls)
        if targetable:
            set_prop(targetable, "interaction_range", 2200.0)
            targetable.configure_from_owner_tags()

    spawn_static(
        "DS_Duct_Tape_Trace_01",
        unreal.Vector(790.0, -566.0, 72.0),
        unreal.Rotator(0.0, 18.0, 0.0),
        unreal.Vector(0.82, 0.06, 0.018),
        cube,
        materials["M_DS_DuctTapeSilver_Proto"],
    )
    spawn_static(
        "DS_Duct_Wake_Ring",
        unreal.Vector(820.0, -520.0, 54.0),
        unreal.Rotator(90.0, 0.0, 0.0),
        unreal.Vector(0.92, 0.92, 0.018),
        cylinder,
        materials["M_DS_DuctWakeGold_Proto"],
    )
    spawn_point_light(
        "DS_Duct_Legend_Glow",
        unreal.Vector(820.0, -520.0, 190.0),
        unreal.Color(255, 188, 28, 255),
        2200.0,
        640.0,
    )

    LEVEL_EDITOR.save_current_level()
    unreal.log("Applied Duct legendary encounter prototype pass.")
